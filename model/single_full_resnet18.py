...
class Net(nn.Module):
    def __init__(self):
        ...
        exact_list=['3', '4', '5', '6']
        self.pre_trained = FeatureExtractor(nn.Sequential(*list(model.children())[:-1]), exact_list)
        # self.pre_trained = new_model(output_layer = 'layer2')
        ...
        self.output_Y = nn.Conv2d(
            in_channels=3, out_channels=1, kernel_size=3, stride=1, padding=1, bias=False)
