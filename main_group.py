...
import feature_extraction_and_vector as vgfe
...
if __name__ == "__main__":
    ...
    for imageQF in opt.imageQFs:
        ...
        for idx_img in range(number_of_Image):
            ...
            print(imageList[idx_img],imageQF)
            ...
            patchPairList, hPatchNum, wPatchNum = utils.create_patchPairList_v3(all_input, all_target,opt.patchSize)

            # allpatch_glcm_feature = utils.glcm_feature(patchPairList,hPatchNum,wPatchNum)
            ft = vgfe.load(patchPairList,hPatchNum,wPatchNum)
            
            patch_group, label = utils.patch_grouping(patchPairList, ft,encodedImageBitLength, imageQF)
            ...
            for groupNum in range(len(patch_group)):
                ...
                if checkpoint.ok:
                    ...
                    if opt.save_pt:
                        t.save('{}_QF{}'.format(imageList[idx_img],imageQF),groupNum)
                    
                    ''' quantize weight '''
                    t.quantize_weight_v4()

                    ''' encode_quanWeight '''
                    encodedWeightBits = checkpoint.encode_quanWeight_v4(t)
                    total_encodedWeightBits += len(encodedWeightBits)

                    ''' encode_quanWeight '''
                    utils.rec_output(t.output,all_output,label_index,wPatchNum,hPatchNum,opt.patchSize)
